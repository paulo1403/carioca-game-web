...
import { getBuyExtrasCount, MAX_BUYS } from "@/utils/buys";
...
export async function handleDrawDiscard(
  ...
  const recentBuyIntents = pendingBuyIntents.filter((i) => Date.now() - i.timestamp < 10000);
  ...
  console.info(
    `[draw] DRAW_DISCARD by ${buyingPlayer.name} (isCurrentPlayer=${isCurrentPlayer}) -> buysUsed=${buyingPlayer.buysUsed}`,
  );
