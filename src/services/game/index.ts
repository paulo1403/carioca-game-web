...
async function forceEmergencyMove(sessionId: string, bot: any) {
  ...
  if (!bot.hasDrawn) {
    ...
    if (refreshedBot && refreshedBot.hand.length > 0) {
      const cardToDiscard = [...refreshedBot.hand].sort(
...
export async function processMove(
  ...
  try {
    ...
    switch (action) {
      case "DRAW_DECK":
        result = await handleDrawDeck(
          session,
          currentPlayer,
          players,
          deck,
          discardPile,
          session.reshuffleCount,
        );
        break;

      case "DRAW_DISCARD":
        result = await handleDrawDiscard(
          session,
          playerId,
          players,
          deck,
