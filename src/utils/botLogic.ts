 ...
 */
interface BotMove {
  action: "DRAW_DECK" | "DRAW_DISCARD" | "DOWN" | "ADD_TO_MELD" | "STEAL_JOKER" | "DISCARD";
...
const decideDraw = (gameState: GameState, bot: Player, difficulty: BotDifficulty): BotMove => {
...
const decideDiscard = (gameState: GameState, bot: Player, _difficulty: BotDifficulty): BotMove => {
  const hand = bot.hand;
...
const findAllPotentialGroups = (
  hand: Card[],
  round: number,
): { trios: CardGroup[]; escalas: CardGroup[] } => {
  const nonJokers = hand.filter((c) => !isJoker(c));
  ...
  const reqs =
    round !== -1
      ...
      : { differentSuitGroups: 1, differentSuitSize: 3, escalas: 1, escalaSize: 3 };
  ...
  if (reqs.differentSuitSize >= 3) {
    ...
    for (const [val, cards] of byValue) {
      ...
      if (naturalCount >= reqs.differentSuitSize) {
        ...
        naturalCount + jokers.length >= reqs.differentSuitSize &&
        naturalCount >= minNaturalRequired
      ) {
        const neededJokers = reqs.differentSuitSize - naturalCount;
        foundTrios.push({
          cards: [...naturalCards, ...jokers.slice(0, neededJokers)],
          type: "TRIO",
        });
      }
    }
    ...
    for (const [suit, cards] of bySuit) {
      for (let start = 1; start <= 13; start++) {
        ...
        if (naturalCount >= minNaturalNeeded && naturalCount + jokers.length >= len) {
          ...
          foundEscalas.push({
            cards: [...resultGroup, ...jokers.slice(0, needed)],
            type: "ESCALA",
          });
        }
      }
    ...
    const neighbors = hand.filter(
      (c) =>
        ...
        (Math.abs(c.value - card.value) <= 2 ||
          (card.value === 1 && c.value >= 12) ||
          (card.value >= 12 && c.value === 1)),
    );
