...
import { canFulfillContract, findPotentialContractGroups } from "@/utils/handAnalyzer";
...
export const BuyConfirmDialog: React.FC<BuyConfirmDialogProps> = ({
  ...
  const analysis = (() => {
    ...
    const willFulfill = canFulfillContract(
    ...
    >
      <div className="space-y-3">
        ...
        <p className="text-sm text-slate-200">
          Solo tienes <span className="font-bold text-yellow-400">{remainingBuys} compras</span>{" "}
          disponibles en toda la partida.
        </p>
        <p className="text-sm text-slate-200">
          Una vez que uses todas tus {MAX_BUYS} compras, no podrás robar más cartas del descarte
        ...
        {analysis && (
          <div className="mt-3">
            ...
            >
              {showReason ? "Ocultar motivo" : "¿Por qué me sugiere esta carta?"}
            </button>

            {showReason && (
              <div className="mt-2 p-3 bg-white/5 rounded-md text-sm text-slate-200">
                <p className="font-semibold">
                  Razón: {analysis.reason || "Ninguna heurística positiva detectada."}
                </p>
                <div className="mt-2">
                  <div className="text-xs text-slate-300 mb-1">
                    Antes: {analysis.before.trios.length} trios, {analysis.before.escalas.length}{" "}
                    escalas
                  </div>
                  <div className="text-xs text-slate-300 mb-2">
                    Después: {analysis.after.trios.length} trios, {analysis.after.escalas.length}{" "}
                    escalas
                  </div>
                  ...
                  {"val" in analysis && (
                    <div className="text-xs text-slate-300 mb-2">
                      Palos para valor {analysis.val}: {analysis.uniqueSuitsBefore} →{" "}
                      {analysis.uniqueSuitsAfter}
                    </div>
                  )}
                  ...
                  {analysis.beforeTriosDesc.length > 0 ? (
                    analysis.beforeTriosDesc.map((s: string, i: number) => (
                      <div key={`btri-${i}`} className="text-xs text-slate-300">
                        {s}
                      </div>
                    ))
                  ) : (
                  ...
                  {analysis.afterTriosDesc.length > 0 ? (
                    analysis.afterTriosDesc.map((s: string, i: number) => (
                      <div key={`atri-${i}`} className="text-xs text-slate-300">
                        {s}
                      </div>
                    ))
                  ) : (
